...
new_bgm_spec = function(
  ...
  stopifnot(
    is.logical(variables$is_continuous),
    length(variables$is_continuous) == 1L
  )
  ...
  stopifnot(
    is.character(missing$na_action),
    length(missing$na_action) == 1L,
  ...
  if (model_type %in% c("omrf", "compare")) {
    stopifnot(
      is.numeric(prior$pairwise_scale),
      length(prior$pairwise_scale) == 1L
    )
  ...
  if (model_type == "mixed_mrf") {
    ...
    stopifnot(is.logical(prior$standardize), length(prior$standardize) == 1L)
    stopifnot(
      is.character(prior$pseudolikelihood),
      length(prior$pseudolikelihood) == 1L
    )
  }
  if (model_type %in% c("ggm", "omrf", "mixed_mrf")) {
    stopifnot(
      is.logical(prior$edge_selection),
      length(prior$edge_selection) == 1L
    )
  ...
  if (model_type == "ggm") {
    stopifnot(
      is.character(prior$pairwise_prior),
      length(prior$pairwise_prior) == 1L
    )
    stopifnot(
      is.numeric(prior$pairwise_scale),
      length(prior$pairwise_scale) == 1L
  ...
  stopifnot(
    is.character(sampler$update_method),
    length(sampler$update_method) == 1L
  )
  stopifnot(
    is.numeric(sampler$target_accept),
    length(sampler$target_accept) == 1L
  )
  ...
  stopifnot(
    is.integer(sampler$hmc_num_leapfrogs),
    length(sampler$hmc_num_leapfrogs) == 1L
  )
  stopifnot(
    is.integer(sampler$nuts_max_depth),
    length(sampler$nuts_max_depth) == 1L
  )
  stopifnot(
    is.logical(sampler$learn_mass_matrix),
    length(sampler$learn_mass_matrix) == 1L
  )
  stopifnot(is.integer(sampler$seed), length(sampler$seed) == 1L)
  stopifnot(
    is.integer(sampler$progress_type),
    length(sampler$progress_type) == 1L
  )
...
validate_bgm_spec = function(spec) {
  ...
  if (mt == "ggm") {
    ...
    if (spec$sampler$update_method != "adaptive-metropolis") {
      stop(
        "bgm_spec: model_type = 'ggm' requires update_method = 'adaptive-metropolis'."
      )
    }
  }
  ...
  if (mt %in% c("ggm", "omrf", "mixed_mrf")) {
    if (
      spec$prior$edge_selection && spec$prior$edge_prior == "Not Applicable"
    ) {
  ...
  if (mt == "mixed_mrf") {
    ...
    if (!(spec$sampler$update_method %in% allowed)) {
      stop(
        ...
        paste(sQuote(allowed), collapse = " or "),
        ". Got '",
...
# ==============================================================================
# bgm_spec_prior_only()  — prior-only sampling spec (GGM)
# ==============================================================================
#
# Constructs a bgm_spec that samples from the prior on the precision matrix,
# with no data contribution. The GGM likelihood term vanishes because n = 0
# and suf_stat = 0. The graph structure is fixed (no edge selection).
# ==============================================================================
bgm_spec_prior_only = function(
  graph,
  pairwise_prior = c("cauchy", "blasso", "normal"),
  pairwise_scale = 2.5,
  blasso_shape = 2,
  ...
  if (is.null(graph) || !is.matrix(graph)) {
    stop("prior_only = TRUE requires 'graph' to be an integer matrix.")
  }
  if (!is.integer(graph)) {
    storage.mode(graph) = "integer"
  }
  p = nrow(graph)
  if (ncol(graph) != p) {
  ...
  data_columnnames = paste0("V", seq_len(p))

  spec = new_bgm_spec(
    model_type = "ggm",
    data = list(
      x = matrix(0, nrow = 0L, ncol = p),
      data_columnnames = data_columnnames,
      num_variables = p,
      num_cases = 0L
    ),
    variables = list(
      variable_type = rep("continuous", p),
    ...
    prior = list(
      ...
      inclusion_probability = matrix(0.5, nrow = p, ncol = p),
      beta_bernoulli_alpha = 1,
      beta_bernoulli_beta = 1,
      beta_bernoulli_alpha_between = 1,
      beta_bernoulli_beta_between = 1,
  ...
  if (na_action == "listwise") {
    ...
    if (n_removed > 0 && isTRUE(getOption("bgms.verbose", TRUE))) {
      ...
      message(
        n_removed,
        " row",
        if (n_removed > 1) "s" else "",
        " with missing values excluded (n = ",
        n_remaining,
        " remaining).\n",
    ...
    if (md_disc$na_impute) {
      missing_index_discrete = md_disc$missing_index
    }
...
build_spec_compare = function(
  ...
  if (!is.null(group_indicator)) {
    ...
    if (length(unique_g) == 0L) {
      stop(
        "The bgmCompare function expects at least two groups, but the input group_indicator contains no group value."
      )
    }
    if (length(unique_g) == 1L) {
      stop(
        "The bgmCompare function expects at least two groups, but the input group_indicator contains only one group value."
      )
    }
    ...
    if (any(tab < 2L)) {
      stop(
        "One or more groups only had one member in the input group_indicator."
      )
    }
  } else {
  ...
  for (variable in seq_len(num_variables)) {
    if (variable > 1) {
      main_effect_indices[variable, 1] = 1L +
        main_effect_indices[variable - 1, 2]
    } else {
  ...
  pairwise_effect_indices = matrix(
    NA_integer_,
    nrow = num_variables,
    ncol = num_variables
  )
...
print.bgm_spec = function(x, ...) {
  ...
  if (s$model_type == "compare") {
    ...
    cat(
      "  difference_selection:",
      s$prior$difference_selection,
      if (s$prior$difference_selection) {
        paste0(" (", s$prior$difference_prior, ")")
      },
      "\n"
    )
  }
