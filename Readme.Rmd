...
graphical models of binary, ordinal, and continuous variables
[@MarsmanVandenBerghHaslbeck_2025].
It supports **ordinal Markov random fields (MRFs)** for discrete data and
**Gaussian graphical models (GGMs)** for continuous data.
...
- `bgm()` – estimates a single network in a one-sample design.
  Use `variable_type = "continuous"` for a GGM, or `"ordinal"` (default)
  for an MRF.
